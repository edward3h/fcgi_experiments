...
public class FCGIInterface 
{
    ...
    */
    ...
    public static UnixServerSocket srvSocket;
    ...
    *
    */
    public int FCGIaccept() throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException
    {
        int acceptResult = 0;
         ...
         */
        if (!acceptCalled){
            isFCGI = true;//System.getProperties().containsKey("FCGI_PORT");
            acceptCalled = true;
            if (isFCGI) {
                /*
...
//                    srvSocket = new ServerSocket(portNum);
                    Constructor<UnixServerSocketChannel> cons = UnixServerSocketChannel.class.getDeclaredConstructor(SelectorProvider.class, int.class);
                    cons.setAccessible(true);
                    UnixServerSocketChannel channel = cons.newInstance(SelectorProvider.provider(), 0);
                    srvSocket = channel.socket();
//                    } catch (IOException e) {
//                        if (request != null)
//                        {
//                            request.socket = null;
